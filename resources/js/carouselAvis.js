...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function adjustCardHeights() {
        ...
        if (window.innerWidth <= 768) {
            slides.forEach(slide => {
                slide.style.height = `${(maxHeight * 3) / 4}px`;
            });
        }
    }
    ...
    // Ajouter l'événement resize pour gérer les changements d'orientation
    window.addEventListener('resize', function() {
        slideWidth = slides[0]?.offsetWidth || 0;
        updateCarousel();  // Rafraîchir l'état du carousel
        adjustCardHeights(); // Ajuster la hauteur des cartes
        toggleReadMoreLinks(); // Réajuster l'affichage des liens "Lire plus"
    });
});
